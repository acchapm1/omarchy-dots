Here's a Bash script template for installing applications on Arch Linux with support for pacman, yay, and Git repositories:
...
1. **Multiple Installation Methods**:
   - Standard packages with `pacman`
   - AUR packages with `yay`
   - Git repositories with `makepkg`

2. **Command Line Options**:
   - `-y` or `--yay`: Use yay instead of pacman
   - `-g` or `--git`: Install from Git repository
   - `-c` or `--clean`: Clean build directory after installation
...
3. **Safety Features**:
   ...
   - Automatic fallback (yay to pacman if not installed)

4. **Usage Examples**:
   ```bash
   # Install standard packages
   ./install.sh htop git vim

   # Install AUR packages with yay
   ./install.sh -y code discord

   # Install from Git repository
   ./install.sh -g https://github.com/user/repo.git

   # Install multiple Git repositories
   ./install.sh -g https://github.com/user/repo1.git -g https://github.com/user/repo2.git
   ```

5. **Clean Operations**:
   - Temporary directories for Git clones
   - Optional cleanup with `-c` flag
   - Proper error handling to avoid orphaned files

### Requirements:
- Arch Linux or Arch-based system
- `pacman` (required)
- `yay` (optional, for AUR packages)
- `git` and `makepkg` (for Git installations)

### Customization:
You can modify the script to:
1. Add support for other AUR helpers (paru, trizen, etc.)
2. Include pre/post installation hooks
3. Add package verification steps
4. Integrate with configuration management
5. Add dependency resolution for Git packages

The script follows best practices for Bash scripting and provides clear feedback during operations.
